...
class SourceHistory {
  ...
  static Future<bool> addHistory(String idUser, String date, String type,
    ...
    if (responseBody['success']) {
      DInfo.notifSuccess('Success', 'Add Success, pls wait a minute');
    } else {
      if (responseBody['message'] == 'date') {
        DInfo.notifError('Oops Error', 'History already create');
      } else {
        DInfo.notifError('Failure', 'Failed to add history');
      }
    }
  ...
  static Future<List<History>> history(String idUser) async {
    ...
    Map? responseBody = await AppRequest.post(url, {'id_user': idUser});
  ...
  static Future<bool> update(String idHistory, String idUser, String date,
    ...
    if (responseBody == null) return false;
    if (responseBody['success']) {
      DInfo.notifSuccess('Success', 'Add Success, Please wait a minute');
    } else {
      if (responseBody['message'] == 'date') {
        DInfo.notifError('Oops', 'Duplicated date, Please check again');
      } else {
        DInfo.notifError('Failure', 'Failed to add history');
      }
    }
